...
app.get("/api/documents/favorites", (req, res) => {
  ...
  const favoriteDocs = documents.filter(
    (doc) => readingRecords[doc.id]?.favorite
  );
  res.json(favoriteDocs);
});
...
app.get("/api/documents/:id", (req, res) => {
  ...
  try {
    ...
    // 获取收藏状态
    const readingRecords = db.get("readingRecords").value();
    const record = readingRecords[id] || { favorite: false };
    res.json({
      ...document,
...
app.post("/api/documents/:id/favorite", (req, res) => {
  ...
  const currentRecord = readingRecords[id] || { reviewCount: 0 };
  db.get("readingRecords")
    .set(id, { ...currentRecord, favorite: true })
    .write();
  res.json({ success: true });
});

// 取消收藏文档
app.post("/api/documents/:id/unfavorite", (req, res) => {
  const { id } = req.params;
  const document = db.get("documents").find({ id }).value();
  if (!document) {
    return res.status(404).json({ error: "文档不存在" });
  ...
  const currentRecord = readingRecords[id] || { reviewCount: 0 };
  db.get("readingRecords")
    .set(id, { ...currentRecord, favorite: false })
    .write();
  res.json({ success: true });
});
